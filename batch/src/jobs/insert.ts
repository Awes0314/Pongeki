...
export async function updateTechFlagList(techFlagList: { title: string; level: string; diff: string; }[]) {
  ...
  // techFlagListが空の場合は処理をスキップ
  if (techFlagList.length === 0) {
    log('info', 'techFlagListが空のため、updateTechFlagList処理をスキップ');
    return;
  }
  ...
  while (i < records.length) {
    ...
    while (retry < MAX_RETRY) {
      ...
      try {
        ...
        log('info', `Processing batch index ${i / BATCH_SIZE + 1}: Existence ${existIdSet.size} / ${ids.length}`);
        ...
        retry++;
        log('warn', `updateTechFlagList: batch index ${i / BATCH_SIZE + 1} リトライ${retry}回目: ${err}`);
        if (retry >= MAX_RETRY) {
          log('error', `updateTechFlagList: batch index ${i / BATCH_SIZE + 1} 失敗: ${err}`);
          throw err;
        }
      }
    }
...
export async function updateChartConstList(chartConstList: { title: string; diff: string; level: string; chartConst: number; ps5Rating: number; ps4Rating: number; ps3Rating: number; ps2Rating: number; ps1Rating: number;}[]) {
  ...
  // chartConstListが空の場合は処理をスキップ
  if (chartConstList.length === 0) {
    log('info', 'chartConstListが空のため、updateChartConstList処理をスキップ');
    return;
  }
  ...
  while (i < records.length) {
    ...
    while (retry < MAX_RETRY) {
      ...
      try {
        ...
        retry++;
        log('warn', `updateChartConstList: batch index ${i / BATCH_SIZE + 1} リトライ${retry}回目: ${err}`);
        if (retry >= MAX_RETRY) {
          log('error', `updateChartConstList: batch index ${i / BATCH_SIZE + 1} 失敗: ${err}`);
          throw err;
        }
      }
    }
...
export async function updateRankingDataList(rankingDataList: { title: string; level: string; diff: string; psTheoryScore: number; ps5Tolerance: number; ps5MinScore: number; tsTheoryCounts: number[]; ps5RainbowCount: number; ps5Count: number; ps4Count: number; ps3Count: number; ps2Count: number; ps1Count: number; psTheoryCount: number;}[]) {
  ...
  if (rankingDataList.length === 0) {
    log('info', 'rankingDataListが空のため、updateRankingDataList処理をスキップ');
    return;
  }
  ...
  while (i < records.length) {
    ...
    while (retry < MAX_RETRY) {
      ...
      try {
        ...
        retry++;
        log('warn', `updateRankingDataList: batch index ${i / BATCH_SIZE + 1} リトライ${retry}回目: ${err}`);
        if (retry >= MAX_RETRY) {
          log('error', `updateRankingDataList: batch index ${i / BATCH_SIZE + 1} 失敗: ${err}`);
          throw err;
        }
      }
    }
